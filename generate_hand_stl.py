...
Featuring organic, ergonomically curved, human-like rounded phalanxes
and palmar tactile pads, while maintaining precise mechanical clevis/pin joints.
Direct CSG and cylinder/icosphere primitives (no external scipy requirement).
"""
...
def generate_intermediate_phalanx(length=28.0, width=13.0, height=11.0):
    ...
    # Anatomical shaft with slight curvature
...
def generate_palm(palm_w=72.0, palm_l=82.0, palm_h=20.0):
    """Palm chassis with curved ergonomic metacarpal arch."""
    palm_core = box(extents=[palm_w - 8, palm_l - 8, palm_h - 4])
    palm_core.apply_translation([0, palm_l / 2, 0])

    # Rounded edge cylinders
    edge_l = cylinder(radius=palm_h * 0.44, height=palm_l - 12, sections=24)
    edge_l.apply_transform(trimesh.transformations.rotation_matrix(np.pi/2, [1, 0, 0]))
    edge_l.apply_translation([-palm_w / 2 + 4, palm_l / 2, 0])

    edge_r = cylinder(radius=palm_h * 0.44, height=palm_l - 12, sections=24)
    edge_r.apply_transform(trimesh.transformations.rotation_matrix(np.pi/2, [1, 0, 0]))
    edge_r.apply_translation([palm_w / 2 - 4, palm_l / 2, 0])

    # Thumb thenar mound
    thumb_boss = icosphere(subdivisions=3, radius=1.0)
    thumb_boss.apply_scale([12.0, 16.0, 11.0])
    ...
    palm_combined = trimesh.boolean.union([palm_core, edge_l, edge_r, thumb_boss])
    ...
    for fx in finger_x:
        ...
        c_slot.apply_translation([fx, palm_l - 2.0, 0])
    ...
    th_pin = cylinder(radius=1.65, height=22.0, sections=24)
    th_pin.apply_transform(trimesh.transformations.rotation_matrix(np.pi/2, [0, 1, 0]))
    th_pin.apply_transform(rot_thumb)
    th_pin.apply_translation([-palm_w / 2 - 2, palm_l * 0.35, -2])
    cutters.append(th_pin)

    # Internal hollow cavity
    cavity = box(extents=[palm_w * 0.65, palm_l * 0.52, palm_h - 6.0])
...
def assemble_hand():
    """Generates the full assembly with curved, organic fingers articulated into a natural posture."""
    print("Generating Ergonomic Palm...")
    palm = generate_palm()
    ...
    # Opposable Thumb
    print("Assembling Rounded Opposable Thumb...")
    ...
    print("Success! All curved STL files regenerated in:", STL_DIR)
